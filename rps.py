...
class PS(object):
    ...
    def solve(self):
        ...
        #############################################
        self.N = np.zeros([65536, 3])
        Norm =np.zeros([65536, 3])

        if self.background_ind is not None:
            for i in range(self.N.shape[1]):
                self.N[self.background_ind, i] = 0
        if self.foreground_ind is not None:
            for j in self.foreground_ind:
                M=np.expand_dims(self.M[j], 1)
                N = np.linalg.lstsq(self.L.T, M, rcond=None)
                N = N[0]
